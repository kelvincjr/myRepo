...
train_text_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/train/input.seq.char'
train_bio_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/train/output.seq.bioattr'
eval_text_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/test/input.seq.char'
eval_bio_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/test/output.seq.bioattr'
schema_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/vocab_attr.txt'
test_file = r'/opt/kelvin/python/knowledge_graph/theta/theta-master-0826/theta/examples/COMMODITY_NER/data/test/test1.txt'
selectedKeys = ['品牌', '品类', '类型']

def load_texts(filename, cond=None):
    D = []
    num_lines = 0
    with open(filename, encoding='utf-8') as f:
        for l in f:
            text = l.replace(' ', '').strip().replace("\n", '').replace(r'[SPA]',' ')
            if cond is None or num_lines in cond:
            	D.append(text)
            num_lines += 1
            if (num_lines % 1000) == 0:
...
def load_bioattr_labels(filename, cond=None):
    D = []
    num_lines = 0
    key_bio_lines = {}
    with open(filename, encoding='utf-8') as f:
            ...
            for tag in tags:
                if tag.startswith('B'):
                    if entity_start:
                        ...
                        if cond is None or entity_type in cond:
                    ...
                    if cur_index == (tags_num - 1):
                        entity_type = tag[2:]
                        entity_start_index = cur_index
                        entity_end_index = cur_index
                        if cond is None or entity_type in cond:
                            entities.append((entity_type, entity_start_index, entity_end_index))
                        entity_start = False
                        entity_start_index = -1
                    ...
                    if entity_start:
                        ...
                        entity_end_index = -1
                        entity_type = ""
                elif tag.startswith('I') and (cur_index == (tags_num - 1)):
                    if entity_start:
                        entity_end_index = cur_index
                        if cond is None or entity_type in cond:
                            entities.append((entity_type, entity_start_index, entity_end_index))
...
def read_schema(filename, cond=None):
    # 读取schema
    with open(filename, encoding='utf-8') as f:
        D = []
        for l in f:
            if l.strip() != 'null' and (cond is None or l.strip() in cond):
                D.append(l.strip())
        return D
...
def train_data_generator(train_text_file, train_bio_file):
    ...
    print("number of lines: ", len(cond))
...
experiment_params = NerAppParams(
    CommonParams(
        ...
        #r"/opt/kelvin/python/knowledge_graph/ai_contest/working",
        r"/opt/kelvin/python/knowledge_graph/ai_contest/working1",
        fp16=False,
        best_index="f1",
        random_type="np"),
