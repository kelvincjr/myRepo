...
class CRF(nn.Module):
    def __init__(self, tagset_size, start_tag_idx, stop_tag_idx):
        ...
        # # We add 2 here, because of self.START_TAG_IDX and self.STOP_TAG_IDX
        ...
        init_transitions[self.STOP_TAG_IDX, :] = -10000.0
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        # partition = partition + self.transitions[self.START_TAG_IDX,:].view(1, tag_size, 1).expand(batch_size, tag_size, 1)
        ...
        # until the last state, add transition score for all partition (and do log_sum_exp) then select the value in self.STOP_TAG_IDX
        cur_values = self.transitions.view(1, tag_size, tag_size).expand(
            batch_size, tag_size, tag_size) + partition.contiguous().view(
                batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
        cur_partition = log_sum_exp(cur_values, tag_size)
        final_partition = cur_partition[:, self.STOP_TAG_IDX]
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for idx, cur_values in seq_iter:
            ...
            cur_values = cur_values + partition.contiguous().view(
                batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
            ## forscores, cur_bp = torch.max(cur_values[:,:-2,:], 1) # do not consider self.START_TAG_IDX/self.STOP_TAG_IDX
        ...
        # exit(0)
        ### add score to final self.STOP_TAG_IDX
        ...
        ## select end ids in self.STOP_TAG_IDX
        pointer = last_bp[:, self.STOP_TAG_IDX]
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        ## transition for label to self.STOP_TAG_IDX
        end_transition = self.transitions[:,
                                          self.STOP_TAG_IDX].contiguous().view(
                                              1, tag_size).expand(
                                                  batch_size, tag_size)
        ...
        ## index the transition score for end_id to self.STOP_TAG_IDX
        ...
        # ## calculate the score from self.START_TAG_IDX to first label
        # start_transition = self.transitions[self.START_TAG_IDX,:].view(1, tag_size).expand(batch_size, tag_size)
    ...
    def _viterbi_decode_nbest(self, feats, mask, nbest):
        ...
        _, inivalues = next(
            seq_iter)  # bat_size * from_target_size * to_target_size
        # only need start from start_tag
        partition = inivalues[:, self.START_TAG_IDX, :].clone(
        ...
        ### calculate the score from last partition to end state (and then select the self.STOP_TAG_IDX from it)
        last_values = last_partition.expand(
        ...
        ## select end ids in self.STOP_TAG_IDX
        pointer = end_bp[:, self.STOP_TAG_IDX, :]  ## (batch_size, nbest)
        ...
        scores = end_partition[:, :, self.STOP_TAG_IDX]
