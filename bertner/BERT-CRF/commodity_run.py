...
train_word_file = config.train_word_file
train_label_file = config.train_label_file
test_word_file = config.test_word_file
...
def load_labels(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for l in f:
            ...
            num_lines += 1
            if (num_lines % 1000) == 0:
                print('{} number of label lines read'.format(num_lines))
    return labels
...
def test():
    word_test = load_words(test_word_file)
    label_test = load_words(test_label_file)
